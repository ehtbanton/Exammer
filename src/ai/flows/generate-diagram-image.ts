 ...
 * - Uses the billed API key (GEMINI_API_KEY_BILLED) for Imagen generation
 */
 ...
 * Uses the billed API key (GEMINI_API_KEY_BILLED) for image generation.
 */
export async function generateDiagramImage(
  ...
  // Get the billed API key from environment
  const apiKey = process.env.GEMINI_API_KEY_BILLED;
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY_BILLED not found in environment variables. Please set it in .env file.');
  }
