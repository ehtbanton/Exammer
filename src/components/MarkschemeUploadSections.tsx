...
interface MarkschemeUploadSectionsProps {
...
export function MarkschemeUploadSections({
  paperTypes,
  subjectId,
  onUpload,
  isLoading,
}: MarkschemeUploadSectionsProps) {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
        <div className="flex items-start gap-2">
          ...
          <div className="text-sm text-blue-800 dark:text-blue-200">
            ...
            <p>
              For automatic matching, include the markscheme date in the filename using the format <code className="bg-blue-100 dark:bg-blue-900 px-1 py-0.5 rounded">yyyy-mm-dd</code> (e.g., "2024-06-15_markscheme.pdf").
              ...
              Markschemes will be matched to existing questions by paper type, year, and question number.
            </p>
          </div>
        </div>
      </div>
      ...
      {paperTypes.map(paperType => {
        ...
        const isMarkschemeLoading = isLoading(`process-markschemes-${subjectId}-${paperType.id}`);
        ...
        return (
          <Card key={paperType.id} className="border-2">
            <CardHeader>
              ...
              <CardDescription>
                Upload markschemes for this paper type. Solutions will be automatically matched to existing questions.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <Button
                ...
                className="w-full"
                disabled={isMarkschemeLoading || files.length === 0}
              >
                {isMarkschemeLoading ? <LoadingSpinner /> : `Process ${files.length} Markscheme${files.length !== 1 ? 's' : ''} & Match`}
              </Button>
            </CardContent>
          </Card>
        );
