...
db.serialize(() => {
  ...
  db.get('SELECT version FROM db_version ORDER BY id DESC LIMIT 1', (err, row) => {
    if (err) console.error('Error getting version:', err.message);
  ...
  db.all("SELECT id, session_type, brainstorm_complete, academic_profile_complete FROM career_sessions ORDER BY created_at DESC LIMIT 1", (err, rows) => {
    if (err) {
